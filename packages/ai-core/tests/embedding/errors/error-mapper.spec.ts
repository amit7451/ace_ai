...
} from '../../../src/embedding/errors/error-mapper';
import {
...
describe('mapHttpStatusToEmbeddingError', () => {
  it('maps 401 and 403 to EmbeddingAuthenticationError', () => {
    expect(mapHttpStatusToEmbeddingError('openai', 401, '')).toBeInstanceOf(
      EmbeddingAuthenticationError
    );
    expect(mapHttpStatusToEmbeddingError('openai', 403, '')).toBeInstanceOf(
      EmbeddingAuthenticationError
    );
  });
  ...
  it('maps 400 and 422 to EmbeddingInvalidRequestError', () => {
    expect(mapHttpStatusToEmbeddingError('gemini', 400, 'bad request')).toBeInstanceOf(
      EmbeddingInvalidRequestError
    );
    expect(mapHttpStatusToEmbeddingError('gemini', 422, 'bad request')).toBeInstanceOf(
      EmbeddingInvalidRequestError
    );
  });

  it('maps 5xx to EmbeddingProviderUnavailableError', () => {
    expect(mapHttpStatusToEmbeddingError('ollama', 503, '')).toBeInstanceOf(
      EmbeddingProviderUnavailableError
    );
  });
});
...
describe('isRetryableEmbeddingError', () => {
  ...
  it('marks authentication and validation errors as non-retryable', () => {
    ...
    expect(isRetryableEmbeddingError(new EmbeddingInvalidRequestError('bad', 'openai'))).toBe(
      false
    );
  });
});
