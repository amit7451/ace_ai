import {
  ...
  toKnowledgeVectorPayload,
} from '../../../src/knowledge/processor/knowledge-processor';
...
describe('KnowledgeProcessor', () => {
  ...
  it('processes plain text end-to-end into KnowledgeChunk objects', () => {
    const chunks = processor.process(
      { content: 'This is a simple knowledge base article about refunds.' },
      baseConfig
    );
  ...
  it('infers html format and defaults sourceType to website', () => {
    const chunks = processor.process(
      { content: '<p>Welcome to our site.</p>', mimeType: 'text/html' },
      baseConfig
    );
  ...
  it('throws KnowledgeEmptyContentError for whitespace-only input', () => {
    expect(() => processor.process({ content: '   ' }, baseConfig)).toThrow(
      KnowledgeEmptyContentError
    );
  });

  it('throws on missing required config fields', () => {
    expect(() =>
      processor.process({ content: 'text' }, { tenantId: '', assistantId: 'a', documentId: 'd' })
    ).toThrow();
  });
  ...
  it('merges a partial chunking override over format defaults', () => {
    ...
    const chunks = processor.process(
      { content: longText },
      { ...baseConfig, chunking: { maxChunkSize: 100, chunkOverlap: 20 } }
    );
...
describe('resolveChunkingOptions', () => {
  ...
  it("rejects a resolved config where overlap >= maxChunkSize (surfaced as a Zod validation error, same as Components 1-3's schema.parse() calls)", () => {
    expect(() =>
      resolveChunkingOptions('plain-text', { maxChunkSize: 100, chunkOverlap: 100 })
