...
describe('LLMProviderFactory', () => {
  ...
  it('throws when given an unsupported provider name', () => {
    expect(() =>
      LLMProviderFactory.create({ provider: 'not-a-real-provider', model: 'x' })
    ).toThrow();
  });
  ...
  it('lists all supported providers', () => {
    ...
    expect(providers).toEqual(
      expect.arrayContaining(['openai', 'anthropic', 'gemini', 'groq', 'openrouter', 'ollama'])
    );
  });
  ...
  it('allows registering a custom provider implementation and restores the original after', () => {
    ...
    const restored = LLMProviderFactory.create({
      provider: 'openai',
      apiKey: 'sk-test',
      model: 'gpt-4o-mini',
    });
