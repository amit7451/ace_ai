...
} from '../../../../src/vector-store/types/vector-store-config.types';
import {
  CollectionInfo,
  CreateCollectionConfig,
} from '../../../../src/vector-store/types/collection.types';
import {
  DeleteResult,
  UpsertResult,
  VectorId,
  VectorRecord,
} from '../../../../src/vector-store/types/vector-record.types';
import {
  VectorFilter,
  VectorSearchQuery,
  VectorSearchResult,
} from '../../../../src/vector-store/types/search.types';
import {
...
class TestVectorStoreProvider extends BaseVectorStoreProvider {
  ...
  protected async rawGetById<TPayload>(
    collection: string,
    ids: VectorId[]
  ...
  protected async rawDeleteByFilter(
    collection: string,
    filter: VectorFilter
...
function makeProvider(overrides: Partial<VectorStoreConfig> = {}): TestVectorStoreProvider {
  return new TestVectorStoreProvider({
    provider: 'qdrant',
...
describe('BaseVectorStoreProvider', () => {
  ...
  describe('getOrCreateCollection', () => {
    ...
    it('returns the existing collection when the vector size matches', async () => {
      ...
      provider.collectionInfoToReturn = {
        name: 'x',
        vectorSize: 4,
        distance: 'cosine',
        pointsCount: 10,
    ...
    it('throws when the existing collection has a different vector size', async () => {
      ...
      await expect(
        provider.getOrCreateCollection({ name: 'x', vectorSize: 1536 })
      ).rejects.toBeInstanceOf(VectorStoreAlreadyExistsError);
    });
  });
