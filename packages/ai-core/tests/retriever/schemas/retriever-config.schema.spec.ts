import {
  ...
  vectorFilterSchema,
} from '../../../src/retriever/schemas/retriever-config.schema';
...
describe('retrievalQuerySchema', () => {
  ...
  it('trims the query and rejects a whitespace-only query', () => {
    expect(() =>
      retrievalQuerySchema.parse({ query: '   ', tenantId: 't1', assistantId: 'a1' })
    ).toThrow();
  });
...
describe('vectorFilterSchema', () => {
  it('accepts a range condition', () => {
    const result = vectorFilterSchema.safeParse({
      must: [{ key: 'chunkIndex', range: { gte: 0, lte: 10 } }],
    });
  ...
  it('accepts a match.any condition', () => {
    const result = vectorFilterSchema.safeParse({
      must: [{ key: 'sourceType', match: { any: ['faq', 'document'] } }],
    });
    expect(result.success).toBe(true);
  });
});
