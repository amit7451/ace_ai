  ...
  protected rawParse(input: RawDocumentInput): ParsedDocument {
    ...
    if (headers.length === 0) {
      sections.push({
        headerPath: [],
        headingLevel: 0,
        content: normalizeWhitespace(stripMarkdownSyntax(input.content)),
      });
    } else {
      if (headers[0].lineIndex > 0) {
        ...
        if (intro.trim()) {
          sections.push({
            headerPath: [],
            headingLevel: 0,
            content: normalizeWhitespace(stripMarkdownSyntax(intro)),
          });
        }
      }
    ...
    const text = normalizeWhitespace(
      nonEmptySections
        .map((s) => (s.headerPath.length ? `${s.headerPath.join(' > ')}\n${s.content}` : s.content))
        .join('\n\n')
    );
