...
export function mapHttpStatusToEmbeddingError(
  ...
  switch (true) {
    ...
    case status === 400 || status === 422:
      return new EmbeddingInvalidRequestError(
        bodyText || `Invalid request to embedding provider "${provider}".`,
        provider
      );
    case status >= 500:
    ...
    default:
      return new EmbeddingError(
        `Unexpected response (${status}) from embedding provider "${provider}": ${bodyText}`,
        provider,
        status
      );
  }
