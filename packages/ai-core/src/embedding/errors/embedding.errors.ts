...
export class EmbeddingAuthenticationError extends EmbeddingError {
  constructor(provider: string, cause?: unknown) {
    super(
      `Authentication failed for embedding provider "${provider}". Check the configured API key.`,
      provider,
      401,
      cause
    );
...
export class EmbeddingTimeoutError extends EmbeddingError {
  constructor(provider: string, timeoutMs: number, cause?: unknown) {
    super(
      `Embedding request to "${provider}" timed out after ${timeoutMs}ms.`,
      provider,
      undefined,
      cause
    );
...
export class EmbeddingBatchSizeError extends EmbeddingError {
  constructor(
    ...
    super(
      `Batch of ${received} inputs exceeds "${provider}"'s configured max batch size of ${maxBatchSize}.`,
      provider,
      400
    );
