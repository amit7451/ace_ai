...
import { EmbeddingAuthenticationError, EmbeddingDimensionMismatchError } from '../../../../src/embedding/errors/embedding.errors';
...
describe('OpenAIEmbeddingProvider', () => {
  ...
  it('automatically chunks batches larger than the configured max batch size and reassembles in order', async () => {
    const makeResponse = (n: number) => ({
      ...
      status: 200,
      json: async () => ({
        data: Array.from({ length: n }, (_, i) => ({ embedding: new Array(1536).fill(0), index: i })),
        model: 'text-embedding-3-small',
    ...
    (global.fetch as jest.Mock).mockResolvedValueOnce(makeResponse(2)).mockResolvedValueOnce(makeResponse(1));
