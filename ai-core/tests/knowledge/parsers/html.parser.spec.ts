...
describe('HtmlParser', () => {
  ...
  it('removes script and style block contents entirely', () => {
    const result = parser.parse({
      content: '<html><head><style>.a{color:red}</style></head><body><script>alert(1)</script><p>Real content</p></body></html>',
    });
    expect(result.text).toBe('Real content');
  });

  it('removes HTML comments', () => {
    const result = parser.parse({ content: '<p>Visible</p><!-- hidden comment --><p>Also visible</p>' });
