...
describe('MarkdownAwareChunkingStrategy', () => {
  ...
  it('prefixes each chunk with its header breadcrumb', () => {
    ...
    expect(result.some((c) => c.metadata?.headerPath && (c.metadata.headerPath as string[]).includes('Installation'))).toBe(true);
  });
  ...
  it('falls back to plain recursive chunking when no markdown structure is present', () => {
    const result = strategy.chunk({ text: 'Just plain text with no headers at all, repeated. '.repeat(10) }, { maxChunkSize: 100, chunkOverlap: 10 });
  ...
  it('does not attach headerPath metadata to the headerless intro section', () => {
    const parsed = parser.parse({ content: ['Intro before any header.', '', '# First', 'Body text.'].join('\n') });
    const result = strategy.chunk(parsed, { maxChunkSize: 500, chunkOverlap: 0 });
