...
describe('OpenAIProvider', () => {
  ...
  it('maps a 401 response to LLMAuthenticationError', async () => {
    global.fetch = jest.fn().mockResolvedValue(errorResponse(401, 'invalid api key')) as unknown as typeof fetch;

    const provider = new OpenAIProvider(config);
    await expect(provider.complete([{ role: 'user', content: 'Hi' }])).rejects.toBeInstanceOf(LLMAuthenticationError);
  });
