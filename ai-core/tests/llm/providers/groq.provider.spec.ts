...
describe('GroqProvider', () => {
  ...
  it('uses the Groq OpenAI-compatible endpoint by default', async () => {
    ...
    const provider = new GroqProvider({ provider: 'groq', apiKey: 'gsk-test', model: 'llama-3.3-70b-versatile' });
  ...
  it('still requires an apiKey (inherited from BaseLLMProvider)', () => {
    expect(() => new GroqProvider({ provider: 'groq', model: 'llama-3.3-70b-versatile' })).toThrow();
  });
});
