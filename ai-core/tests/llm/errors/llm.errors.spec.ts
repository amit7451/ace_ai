...
describe('LLM error classes', () => {
  it('carries provider/model/statusCode context on the base fields', () => {
    const error = new LLMTimeoutError({ provider: 'openai', model: 'gpt-4o-mini', statusCode: 408 });
  ...
  it('LLMProviderUnavailableError and LLMContextLengthError set the expected codes', () => {
    expect(new LLMProviderUnavailableError({ provider: 'gemini' }).code).toBe('PROVIDER_UNAVAILABLE_ERROR');
