...
describe('retryWithBackoff', () => {
  ...
  it('throws immediately for a non-retryable error', async () => {
    ...
    await expect(retryWithBackoff(fn, { maxRetries: 3, baseDelayMs: 1 })).rejects.toBeInstanceOf(LLMAuthenticationError);
    expect(fn).toHaveBeenCalledTimes(1);
  });
  ...
  it('throws after exhausting all retries', async () => {
    ...
    await expect(retryWithBackoff(fn, { maxRetries: 2, baseDelayMs: 1 })).rejects.toBeInstanceOf(LLMRateLimitError);
  ...
  it('respects a custom isRetryable predicate', async () => {
    ...
    await expect(retryWithBackoff(fn, { maxRetries: 3, baseDelayMs: 1, isRetryable })).rejects.toThrow('custom error');
    expect(fn).toHaveBeenCalledTimes(1);
  ...
  it('calls onRetry with the attempt number and computed delay', async () => {
    ...
    const fn = jest.fn().mockRejectedValueOnce(new LLMRateLimitError({ provider: 'test' })).mockResolvedValueOnce('ok');
