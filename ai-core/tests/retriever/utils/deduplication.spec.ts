...
describe('deduplicateByText', () => {
  it('keeps the first occurrence and drops exact-duplicate text', () => {
    const results = [result('a', 'Our refund policy allows returns within 30 days.'), result('b', 'Our refund policy allows returns within 30 days.')];
  ...
  it('treats whitespace and case differences as the same text', () => {
    const results = [result('a', 'Refund Policy: 30 days.'), result('b', '  refund   policy:   30 days.  ')];
    const deduped = deduplicateByText(results);
  ...
  it('preserves original rank order among kept results', () => {
    const results = [result('a', 'First', 0.95), result('b', 'Second', 0.8), result('c', 'Third', 0.6)];
    const deduped = deduplicateByText(results);
    expect(deduped.map((r) => r.id)).toEqual(['a', 'b', 'c']);
  });

  it('passes through results with no payload text unaffected', () => {
    const results: VectorSearchResult<KnowledgeVectorPayload>[] = [{ id: 'a', score: 0.9 }, { id: 'b', score: 0.8 }];
