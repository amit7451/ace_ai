import { filterByThreshold, isLowerScoreBetter } from '../../../src/retriever/utils/threshold-filter';
...
describe('filterByThreshold', () => {
  it('keeps only candidates at or above the threshold for cosine, sorted best-first', () => {
    const candidates = [candidate('a', 0.4), candidate('b', 0.9), candidate('c', 0.6), candidate('d', 0.2)];
  ...
  it('keeps only candidates at or below the threshold for euclid, sorted best-first (ascending)', () => {
    const candidates = [candidate('a', 5), candidate('b', 1), candidate('c', 3), candidate('d', 10)];
