...
describe('RerankStrategyFactory', () => {
  ...
  it('throws RetrieverUnsupportedStrategyError for an unknown strategy name', () => {
    ...
    expect(() => RerankStrategyFactory.create('not-a-real-strategy')).toThrow(RetrieverUnsupportedStrategyError);
  });

  it('lists all supported strategies', () => {
    expect(RerankStrategyFactory.getSupportedStrategies()).toEqual(expect.arrayContaining(['similarity-threshold', 'mmr']));
  });
  ...
  it('allows registering a custom strategy implementation and restores the original after', () => {
    ...
    expect(RerankStrategyFactory.create('similarity-threshold')).toBeInstanceOf(SimilarityThresholdRerankStrategy);
  });
});
