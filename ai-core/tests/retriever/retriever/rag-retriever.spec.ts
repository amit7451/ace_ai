...
function collectionInfo(overrides: Partial<CollectionInfo> = {}): CollectionInfo {
  return { name: 'assistant_abc123', vectorSize: 3, distance: 'cosine', pointsCount: 42, status: 'green', ...overrides };
}
...
function searchResult(id: string, score: number, overrides: Partial<KnowledgeVectorPayload> = {}): VectorSearchResult<KnowledgeVectorPayload> {
...
describe('RagRetriever', () => {
  it('embeds the query with inputType "query" and returns ranked chunks', async () => {
    ...
    const vectorStore = fakeVectorStore({
      search: jest.fn().mockResolvedValue([searchResult('c1', 0.9), searchResult('c2', 0.8, { text: 'Different text entirely.' })]),
    });

    const retriever = new RagRetriever(embedding, vectorStore, { collection: 'assistant_abc123' });
    const result = await retriever.retrieve({ query: 'What is your refund policy?', tenantId: 'tenant_1', assistantId: 'assistant_abc123' });

    expect(embedding.embed).toHaveBeenCalledWith('What is your refund policy?', { inputType: 'query' });
  ...
  it('always ANDs tenantId and assistantId into the search filter', async () => {
    ...
    const retriever = new RagRetriever(fakeEmbeddingProvider(), fakeVectorStore({ search: searchMock }), { collection: 'c' });
  ...
  it('returns isRelevant: false and an empty chunk list when nothing clears the score threshold', async () => {
    const vectorStore = fakeVectorStore({ search: jest.fn().mockResolvedValue([searchResult('c1', 0.1)]) });
    const retriever = new RagRetriever(fakeEmbeddingProvider(), vectorStore, { collection: 'c', scoreThreshold: 0.5 });

    const result = await retriever.retrieve({ query: 'what is quantum physics?', tenantId: 'tenant_1', assistantId: 'assistant_abc123' });
  ...
  it('throws RetrieverDimensionMismatchError on every call while the mismatch persists (not cached as a permanent failure)', async () => {
    ...
    await expect(retriever.retrieve({ query: 'hi', tenantId: 't1', assistantId: 'a1' })).rejects.toBeInstanceOf(RetrieverDimensionMismatchError);
    await expect(retriever.retrieve({ query: 'hi again', tenantId: 't1', assistantId: 'a1' })).rejects.toBeInstanceOf(RetrieverDimensionMismatchError);
  ...
  it('caches a successful dimension verification so later calls skip the extra round trip', async () => {
    ...
    const vectorStore = fakeVectorStore({ getCollectionInfo: getCollectionInfoMock, search: jest.fn().mockResolvedValue([]) });
    const retriever = new RagRetriever(fakeEmbeddingProvider(), vectorStore, { collection: 'assistant_abc123' });
  ...
  it('requests candidate vectors only when the configured strategy needs them', async () => {
    const searchMock = jest.fn().mockResolvedValue([]);
    const defaultRetriever = new RagRetriever(fakeEmbeddingProvider(), fakeVectorStore({ search: searchMock }), { collection: 'c' });
    ...
    const mmrRetriever = new RagRetriever(fakeEmbeddingProvider(), fakeVectorStore({ search: searchMock }), {
      collection: 'c',
      strategy: 'mmr',
    });
  ...
  it('accepts an injected custom rerank strategy instead of using the factory', async () => {
    ...
    const retriever = new RagRetriever(fakeEmbeddingProvider(), fakeVectorStore(), { collection: 'c' }, customStrategy);
  ...
  it('deduplicates near-identical chunk text across results', async () => {
    ...
    const vectorStore = fakeVectorStore({
      search: jest.fn().mockResolvedValue([searchResult('c1', 0.9, { text: sameText }), searchResult('c2', 0.8, { text: sameText })]),
    });
    const retriever = new RagRetriever(fakeEmbeddingProvider(), vectorStore, { collection: 'c' });

    const result = await retriever.retrieve({ query: 'refund?', tenantId: 't1', assistantId: 'a1' });
  ...
  it('trims to maxContextTokens when configured', async () => {
    ...
    const retriever = new RagRetriever(fakeEmbeddingProvider(), vectorStore, { collection: 'c', maxContextTokens: 150 });
  ...
  it('per-query topK/scoreThreshold override the retriever-level config', async () => {
    ...
    const retriever = new RagRetriever(fakeEmbeddingProvider(), fakeVectorStore({ search: searchMock }), {
      collection: 'c',
      scoreThreshold: 0.9,
    });

    const result = await retriever.retrieve({ query: 'hi', tenantId: 't1', assistantId: 'a1', scoreThreshold: 0.5 });
  ...
  describe('healthCheck', () => {
    it('returns true when both dependencies are healthy', async () => {
      const retriever = new RagRetriever(fakeEmbeddingProvider(), fakeVectorStore(), { collection: 'c' });
      await expect(retriever.healthCheck()).resolves.toBe(true);
    });

    it('returns false when the embedding provider is unhealthy', async () => {
      const retriever = new RagRetriever(fakeEmbeddingProvider({ healthCheck: jest.fn().mockResolvedValue(false) }), fakeVectorStore(), {
        collection: 'c',
      });
    ...
    it('never throws, even if a dependency healthCheck rejects', async () => {
      const retriever = new RagRetriever(
        fakeEmbeddingProvider({ healthCheck: jest.fn().mockRejectedValue(new Error('network down')) }),
        fakeVectorStore(),
        { collection: 'c' },
      );
      await expect(retriever.healthCheck()).resolves.toBe(false);
