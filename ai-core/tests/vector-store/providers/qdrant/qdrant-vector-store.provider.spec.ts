...
describe('QdrantVectorStoreProvider', () => {
  ...
  describe('createCollection', () => {
    it('sends a PUT to create the collection, then fetches its info', async () => {
      ...
      expect(info).toEqual({ name: 'docs', vectorSize: 4, distance: 'cosine', pointsCount: 42, status: 'green' });
  ...
  describe('error mapping', () => {
    it('maps a 401 response to VectorStoreAuthenticationError', async () => {
      ...
      const provider = createProvider();
      await expect(provider.getCollectionInfo('docs')).rejects.toBeInstanceOf(VectorStoreAuthenticationError);
    });

    it('maps a fetch rejection to VectorStoreConnectionError', async () => {
      fetchMock.mockRejectedValueOnce(new Error('ECONNREFUSED'));
      const provider = createProvider();
      await expect(provider.getCollectionInfo('docs')).rejects.toBeInstanceOf(VectorStoreConnectionError);
    });
  ...
  describe('upsertBatch', () => {
    it('validates vector dimensions against the collection before upserting', async () => {
      ...
      const provider = createProvider();
      await expect(provider.upsertBatch('docs', [{ id: 1, vector: [1, 2, 3] }])).rejects.toBeInstanceOf(
        VectorStoreDimensionMismatchError
      );
    });
  ...
  describe('search', () => {
    it('sends the query vector and maps scored points back', async () => {
      ...
      expect(results).toEqual([{ id: 'a', score: 0.9, payload: { text: 'hello' }, vector: undefined }]);
  ...
  describe('delete / count', () => {
    ...
    it('counts points matching a filter', async () => {
      ...
      const provider = createProvider();
      const result = await provider.count('docs', { must: [{ key: 'tenantId', match: { value: 't1' } }] });
