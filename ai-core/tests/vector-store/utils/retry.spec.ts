...
import { VectorStoreConnectionError, VectorStoreAuthenticationError } from '../../../src/vector-store/errors/vector-store.errors';
...
describe('withTimeout', () => {
  it('resolves normally when the promise finishes before the timeout', async () => {
    const result = await withTimeout(Promise.resolve('fast'), 1000, () => new Error('should not fire'));
