...
describe('VectorStoreProviderFactory', () => {
  it('creates a QdrantVectorStoreProvider for a valid qdrant config', () => {
    const store = VectorStoreProviderFactory.create({ provider: 'qdrant', url: 'http://localhost:6333' });
  ...
  it('throws VectorStoreInvalidRequestError for an invalid url', () => {
    expect(() => VectorStoreProviderFactory.create({ provider: 'qdrant', url: 'not-a-url' })).toThrow(
