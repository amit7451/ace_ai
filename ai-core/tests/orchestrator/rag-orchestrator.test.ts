...
describe('RagOrchestrator', () => {
  ...
  it('should successfully orchestrate a chat turn', async () => {
    ...
    // 1. Mock retrieval
    const fakeRetrievalResult = { isRelevant: true, chunks: [], query: request.query, totalCandidates: 0, tookMs: 0 };
    ...
    mockPromptBuilder.buildPrompt.mockResolvedValue([{ role: 'system', content: 'test' }, { role: 'user', content: 'What is the refund policy?' }]);
    ...
    expect(mockRetriever.retrieve).toHaveBeenCalledWith({ query: request.query, tenantId: request.tenantId, assistantId: request.assistantId });
  ...
  it('should short-circuit and return fallback on OutOfDomainError', async () => {
    ...
    mockRetriever.retrieve.mockResolvedValue({ isRelevant: false, chunks: [], query: request.query, totalCandidates: 0, tookMs: 0 });
