...
function recursiveSplitToUnits(text: string, levelIndex: number, maxChunkSize: number): SplitUnit[] {
...
function mergeUnitsWithOverlap(units: SplitUnit[], maxChunkSize: number, chunkOverlap: number): string[] {
  const chunks: string[] = [];
  let current = '';
 ...
 */
export function splitRecursively(text: string, maxChunkSize: number, chunkOverlap: number): string[] {
