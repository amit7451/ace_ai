...
function hasVector<TPayload>(candidate: VectorSearchResult<TPayload>): candidate is CandidateWithVector<TPayload> {
...
export class MmrRerankStrategy implements IRerankStrategy {
  ...
  rerank<TPayload = KnowledgeVectorPayload>(
    ...
    const eligible = filterByThreshold(candidates, context.scoreThreshold, context.distanceMetric).filter(hasVector);
    ...
    while (selected.length < context.topK && remaining.length > 0) {
      ...
      for (let i = 0; i < remaining.length; i++) {
        ...
        const maxSimToSelected =
          selected.length === 0 ? 0 : Math.max(...selected.map((s) => cosineSimilarity(candidate.vector, s.vector)));
        const mmrScore = context.mmrLambda * candidate.score - (1 - context.mmrLambda) * maxSimToSelected;
