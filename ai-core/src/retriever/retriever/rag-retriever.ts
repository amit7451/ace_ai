...
export class RagRetriever implements IRetriever {
  ...
  async retrieve(rawQuery: RetrievalQuery): Promise<RetrievalResult> {
    ...
    const searchTopK = Math.max(topK * SEARCH_OVER_FETCH_MULTIPLIER, topK + SEARCH_OVER_FETCH_MINIMUM);

    const candidates = await this.vectorStore.search<KnowledgeVectorPayload>(this.config.collection, {
      vector: queryVector,
      topK: searchTopK,
      filter,
      withPayload: true,
