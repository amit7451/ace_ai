...
- **Normalized errors** — Qdrant's HTTP error responses are mapped onto
  ...
  - `VectorStoreDimensionMismatchError` — the write-side counterpart to
    Component 2's `EmbeddingDimensionMismatchError`. Thrown *before* a
...
No new dependencies. This layer reuses `zod` (already added in Component
2) for config validation and native `fetch` (already relied on by
...
- As with Components 1 and 2's retry logic, only the *initial* request is
