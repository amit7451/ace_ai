...
import { VectorStoreAlreadyExistsError, VectorStoreTimeoutError } from '../../errors/vector-store.errors';
...
export abstract class BaseVectorStoreProvider implements IVectorStore {
  ...
  private async execute<T>(fn: () => Promise<T>, timeoutMessage: string): Promise<T> {
    return retryWithBackoff(
      () =>
        withTimeout(fn(), this.timeoutMs, () => new VectorStoreTimeoutError(timeoutMessage, { provider: this.provider })),
  ...
  async deleteCollection(name: string): Promise<void> {
    await this.execute(() => this.rawDeleteCollection(name), `Timed out deleting collection "${name}"`);
    this.dimensionCache.delete(name);
  }

  async collectionExists(name: string): Promise<boolean> {
    return this.execute(() => this.rawCollectionExists(name), `Timed out checking collection "${name}"`);
  }

  async getCollectionInfo(name: string): Promise<CollectionInfo> {
    const info = await this.execute(() => this.rawGetCollectionInfo(name), `Timed out fetching collection "${name}"`);
    this.dimensionCache.set(info.name, info.vectorSize);
    return info;
  }
  ...
  async upsertBatch<TPayload = KnowledgeVectorPayload>(
    ...
    for (const record of records) {
      validateVectorDimension(record.vector, expectedSize, this.provider, `upsert into "${collection}"`);
    }
  ...
  async search<TPayload = KnowledgeVectorPayload>(
    ...
    return this.execute(() => this.rawSearch<TPayload>(collection, normalized), `Timed out searching "${collection}"`);
  }
  ...
  async count(collection: string, filter?: VectorFilter): Promise<number> {
    return this.execute(() => this.rawCount(collection, filter), `Timed out counting points in "${collection}"`);
  }
  ...
  protected abstract rawGetById<TPayload>(collection: string, ids: VectorId[]): Promise<VectorRecord<TPayload>[]>;
  protected abstract rawDelete(collection: string, ids: VectorId[]): Promise<DeleteResult>;
  protected abstract rawDeleteByFilter(collection: string, filter: VectorFilter): Promise<DeleteResult>;
