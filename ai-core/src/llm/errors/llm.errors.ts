...
export class LLMAuthenticationError extends LLMError {
  constructor(context: LLMErrorContext) {
    super(`Authentication failed for provider "${context.provider}". Check the configured API key.`, 'AUTHENTICATION_ERROR', context);
...
export class LLMProviderUnavailableError extends LLMError {
  constructor(context: LLMErrorContext) {
    super(`Provider "${context.provider}" is currently unavailable. Please retry shortly.`, 'PROVIDER_UNAVAILABLE_ERROR', context);
...
export class LLMContextLengthError extends LLMError {
  constructor(context: LLMErrorContext) {
    super(`Request to provider "${context.provider}" exceeded the model's maximum context length.`, 'CONTEXT_LENGTH_ERROR', context);
...
export function mapHttpStatusToError(status: number, provider: string, message: string, model?: string): LLMError {
  switch (status) {
    case 401:
    case 403:
    ...
    case 404:
    case 422:
      return new LLMInvalidRequestError(message || `Invalid request sent to provider "${provider}".`, {
        provider,
        model,
        statusCode: status,
      });
    ...
    default:
      return new LLMUnknownError(message || `Unexpected error from provider "${provider}".`, { provider, model, statusCode: status });
  }
}
...
export async function mapHttpErrorResponse(response: HttpErrorLike, provider: string, model?: string): Promise<LLMError> {
