...
export class GeminiProvider extends BaseLLMProvider {
  ...
  async *stream(messages: LLMMessage[], options?: LLMCompletionOptions): AsyncGenerator<LLMStreamChunk, void, unknown> {
    ...
    if (!response.body) {
      throw new LLMInvalidRequestError('Gemini stream response had no body.', { provider: this.name, model: this.model });
    }
  ...
  async healthCheck(): Promise<boolean> {
    try {
      const response = await fetch(`${this.baseUrl}/models/${this.model}`, { headers: this.buildHeaders(this.authHeaders()) });
