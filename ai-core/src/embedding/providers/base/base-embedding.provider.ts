import { EmbeddingConfig, EmbeddingInputType, EmbeddingProviderName, ResolvedEmbeddingConfig } from '../../types/embedding-config.types';
...
import { EmbeddingDimensionMismatchError, EmbeddingInvalidRequestError, EmbeddingTimeoutError } from '../../errors/embedding.errors';
...
export abstract class BaseEmbeddingProvider implements IEmbeddingProvider {
  ...
  async embedBatch(inputs: string[], options?: EmbedOptions): Promise<EmbeddingResponse> {
    ...
    for (const batch of batches) {
      const result = await retryWithBackoff(() => this.executeWithTimeout(() => this.rawEmbed(batch, inputType)), {
  ...
  protected abstract rawEmbed(inputs: string[], inputType: EmbeddingInputType): Promise<EmbeddingResponse>;
}
